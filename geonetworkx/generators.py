...
def extended_ego_graph(G: GeoGraph, n, radius=1, center=True, undirected=False, distance=None) -> GeoGraph:
    ...
    the ``settings.ORIGINAL_EDGE_KEY`` attribute that is set on boundary edges. Moreover, the attribute ``distance`` is
    filled with the value :math:`d(u, b)`.
    """
